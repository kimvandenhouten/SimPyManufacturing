...
collected_data = []
n_sim = 10
results_location = f'results.csv'
for (instance_size, instance_id) in [(10, 1), (10, 2), (20, 1), (20, 2)]:
        for instance_type in [2]:
            ...
            my_productionplan = ProductionPlan(
                **json.load(open(f'dsm_experiments/factory_data/development/instances_type_{instance_type}_uniform/instance_' + instance_name + '.json')))
            ...
            if res:
                ...
                stnu = FACTORY_STNU.from_production_plan(my_productionplan, max_time_lag=True, origin_horizon=False)
                stnu = add_resource_chains(stnu=stnu, resource_chains=resource_chains)
                stnu_to_xml(stnu, f"input_production_plan_{instance_size}_{instance_id}", "temporal_networks/cstnu_tool/xml_files")

                # Run DC-checking and store ESTNU in xml
                dc, output_location = run_dc_algorithm("temporal_networks/cstnu_tool/xml_files", f"input_production_plan_{instance_size}_{instance_id}")
