...
from temporal_networks.cstnu_tool.call_java_cstnu_tool import run_dc_algorithm, run_rte_algorithm
...
def run_instance(instance_id):
    ...
    if not res:
        ...
        stnu = RCPSP_STNU.from_rcpsp_max_instance(durations, temporal_constraints, sink_source=1)
        stnu = add_resource_chains(stnu, resource_chains)
        stnu_to_xml(stnu, f"example_rcpsp_max_stnu_java", "temporal_networks/cstnu_tool/xml_files")
        ...
        start = time.time()
        dc, output_location = run_dc_algorithm("temporal_networks/cstnu_tool/xml_files", f"example_rcpsp_max_stnu_java")
