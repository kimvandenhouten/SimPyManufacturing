    ...
    for instance_id in [1]:
        for instance_type in [1, 2]:
            ...
            for i in range(n_sim):
                ...
                # TODO: make estnu again
                earliest_start = cp_output.to_dict('records')
                resource_chains, resource_use = get_resource_chains(my_productionplan, earliest_start, True)
                # Set up STNU and write to xml
                stnu = STNU.from_production_plan(my_productionplan, max_time_lag=True, origin_horizon=False)
                stnu = add_resource_chains(stnu=stnu, resource_chains=resource_chains)
                stnu_to_xml(stnu, f"input_production_plan_{instance_size}_{instance_id}",
                            "stnu/java_comparison/xml_files")

                # Run DC-checking and store ESTNU in xml
                dc, output_location = run_dc_algorithm("stnu/java_comparison/xml_files",
                                                       f"input_production_plan_{instance_size}_{instance_id}")
                logger.info(f'dc is {dc} and output location {output_location}')
                ...
                # TODO: feasibility check on the output schedule
                makespan_stnu_new = max(rte_data.f.values())
                logger.info(f'The makespan obtained with the new "optimal" STNU algorithm is {makespan_stnu_new}')
                ...
                collected_data.append(
                    {"instance_type": instance_type, "instance": instance_name, "it": i, "makespan_lb": makespan_pi, "makespan_stnu": makespan_stnu,
                     "regret": makespan_stnu - makespan_pi, "makespan_new": makespan_stnu_new, "regret_new": makespan_stnu_new - makespan_pi})
                collected_data_df = pd.DataFrame(collected_data)
                collected_data_df.to_csv(f'stnu/experiments/results_with_optimal_stnu.csv')
