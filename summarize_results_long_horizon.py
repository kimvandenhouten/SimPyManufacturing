                ...
                for l1 in [0.5, 0.9]:
                    ...
                    for (k, m) in [(40, 10)]:
                        for search_method in ["local_search"]:
                            ...
                            setting = Settings(method=f"{decompose}_{search_method}",
                                               instance=f'{size}_{id}_{factory_name}',
                                               size=size, simulator=simulator, stop_criterium="Budget",
                                               budget=(size / 20) * 200,
                                               objective=f'l1={l1}_l2={l2}', init="random", seed=seed, l1=l1, l2=l2, k=k,
                                               m=m)
                            settings_list.append(setting)
...
results.to_csv("summary_tables/long horizon global search seed 1-3, rolling horizon seed 1-3.csv")
